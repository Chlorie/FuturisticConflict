...
namespace fc::oth
{
    ...
    namespace
    {
        ...
        bool bit_test(const BitBoard bits, const int row, const int column)
        {
        ...
        {
            const BitBoard top_mask = 0xff00000000000000ull;
            const BitBoard left_mask = 0x8080808080808080ull;
            const BitBoard bottom_mask = 0x00000000000000ffull;
            const BitBoard right_mask = 0x0101010101010101ull;
            const BitBoard invert_left_mask = 0x7f7f7f7f7f7f7f7full;
            const BitBoard invert_right_mask = 0xfefefefefefefefeull;
            const BitBoard self = black ? state.black : state.white;
            const BitBoard occupied = state.black | state.white;
            const BitBoard corners = self & 0x8100000000000081ull; // Corners occupied by [self]
        ...
        ActionValue weighted_search(const State& state, const BitBoard playable, const bool black,
            const bool maximizer,
                    ...
                    if (bit_test(playable, i, j)) // Test every child node
                    {
                        ...
                        // Get current value, which includes deeper search
                        {
                            ...
                            if (result == OthelloLogic::Result::not_finished)
                            {
                                ...
                                current_value = weighted_search(current_state, current_playable, game.is_black(),
                                    maximizer,
                                    depth - 1, alpha, beta).value;
                            }
                            else if (result == OthelloLogic::Result::black_win)
                                current_value = maximizer ? win_value : -win_value;
                            else if (result == OthelloLogic::Result::white_win)
                                current_value = maximizer ? -win_value : win_value;
                            else
                                current_value = 0;
                        }
        ...
        EndGameResult exhaustive_search(const State& state, const BitBoard playable, const bool black,
            const bool maximizer,
