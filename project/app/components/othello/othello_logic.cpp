...
namespace fc
{
    OthelloLogic::Result OthelloLogic::get_result() const
    {
        ...
        if (black > white) return Result::black_win;
        if (black < white) return Result::white_win;
        return Result::draw;
    }
    ...
    OthelloLogic::BitBoard OthelloLogic::compute_playable_spots(const State& state, const bool black)
    {
        ...
        const BitBoard center = opponent & 0x007e7e7e7e7e7e00ull; // Center 6x6
        const BitBoard rows = opponent & 0x7e7e7e7e7e7e7e7eull; // Middle 6 rows
        ...
        return (northwest | southeast | north | south | northeast | southwest | west | east) & empty;
    }
    ...
    void OthelloLogic::reverse(const int row, const int column)
    {
        ...
        const BitBoard new_disk = 0x8000000000000000ull >> (row * 8 + column); // Newly placed disk
        // Apply masks to opponent bit board to avoid row wrapping in the left/right shifts
        const BitBoard center = opponent & 0x007e7e7e7e7e7e00ull; // Center 6x6
        const BitBoard rows = opponent & 0x7e7e7e7e7e7e7e7eull; // Middle 6 rows
        // Initialize the result of the 8 directions, with the first iteration done
